...
import { completeOnboarding, getProfile, getPublicProfile, syncPhoneVerification, updateProfile } from './service.js';
import { completeOnboardingSchema, syncPhoneVerificationSchema, updateProfileSchema } from './validations.js';
...
export const completeMyOnboarding = [
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const updated = await completeOnboarding(req.auth!.userId, parsed);
    if (!updated) throw new AppError('User not found', 404, 'USER_NOT_FOUND');
    res.json({ data: updated });
  }),
];
