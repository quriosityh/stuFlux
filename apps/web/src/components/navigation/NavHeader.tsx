...
export function NavHeader() {
  ...
  return (
    <nav
      ...
      className={cn(
        "!fixed top-0 left-0 right-0 z-50 bg-zinc-50 dark:bg-zinc-900 border-b border-border/10 shadow-sm transition-all duration-300",
        pathname.startsWith('/messages') ? "hidden md:block" : "block"
      )}
          ...
          <div className="flex items-center justify-end gap-3 w-48 z-20 shrink-0">
            {!userId ? (
              <div className="flex items-center gap-3">
                ...
                <Link
                  href="/auth/sign-up"
                  className="text-sm font-semibold bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 px-4 py-2 rounded-full hover:opacity-90 transition-opacity whitespace-nowrap"
                >
                  Sign up
                </Link>
              </div>
              ...
              <>
                <div className="relative flex items-center justify-end h-10 mr-2">
                  ...
                  <AnimatePresence>
                    {showNavMenu && !isExpanded && (
                      <motion.div
                        initial={{ opacity: 0, y: 10, scale: 0.95 }}
                        animate={{ opacity: 1, y: 0, scale: 1 }}
                        exit={{ opacity: 0, y: 10, scale: 0.95 }}
                        className="absolute top-12 right-0 w-48 rounded-2xl bg-background border border-border/40 shadow-2xl p-2 z-50 flex flex-col"
                      >
                        {navItems.map((item) => (
                          <Link
                            key={item.name}
                            ...
                            className={cn(
                              "px-4 py-3 rounded-xl text-sm font-semibold hover:bg-muted/60 transition-colors",
                              pathname === item.href ? "text-[var(--accent)]" : "text-foreground/80"
                            )}
                          >
                            {item.name}
                          </Link>
                ...
                <Link
                  ...
                  id="nav-profile-link"
                  className="relative group z-10 w-10 h-10 shrink-0 aspect-square rounded-full border border-border/20 overflow-hidden hover:ring-2 hover:ring-[var(--accent)] transition-all cursor-pointer bg-gradient-to-br from-background to-surface flex items-center justify-center"
                >
                  {(profileAvatar || user?.imageUrl) ? (
                    <img src={profileAvatar || user?.imageUrl} alt="Profile" className="w-full h-full object-cover rounded-full" />
                  ) : (
